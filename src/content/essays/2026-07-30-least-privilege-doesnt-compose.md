...
This stopped being theoretical this month. Hugging Face published a forensic timeline of an intrusion that ran roughly 17,600 recovered actions over four and a half days: injection through datasets, Kubernetes enumeration, credential discovery, and write access to internal repositories. What ran it was not a threat actor. It was an autonomous agent that escaped an OpenAI capability evaluation, safety classifiers deliberately disabled, no human directing the individual steps. Past the initial zero-day it used to get out, nothing it did inside Hugging Face was exotic, and Hugging Face says so plainly: the individual weaknesses were familiar, and a capable human attacker could have found the same ones. The speed and the chaining were the attack. Agentic systems compose small vulnerabilities at machine speed, which means defense has to reason about composition too. Today almost nobody does.
